...
// Funciones para redondear y actualizar el menú
int round_to_int(float value) {
    return (int)(value + 0.5);  // Redondea al entero más cercano
}

void mostrar_menu_pid() {
    lcd_gotoxy(1, 1);
    printf(lcd_putc, "Kp_x:%0.2f Ki_x:%0.2f", Kp_x, Ki_x); // Muestra Kp y Ki en la primera línea
    lcd_gotoxy(1, 2);
    printf(lcd_putc, "Kd_x:%0.2f SETPOINT:0", Kd_x);       // Muestra Kd en la segunda línea
}

void ajustar_pid() {
    // Aquí puedes añadir el código de lógica de entrada para ajustar Kp, Ki, y Kd
    // con botones o entradas específicas
    // Ejemplo: incremento/decremento de Kp_x con botones específicos
}
...
void main() {
    ...
    mostrar_menu_pid();  // Muestra el menú de PID
    ...
    while (true) {
        ...
        error_x = 0 - Gx;  // Error para el servo 1 (setpoint es 0)
        error_y = 0 - Gy;  // Error para el servo 2 (setpoint es 0)
        ...
        // Calcular ángulos para los servos basados en la salida del PID, redondeados a enteros
        angle_x = 90 + round_to_int(output_x);  // Calcular ángulo del servo 1
        angle_y = 90 - round_to_int(output_y);  // Calcular ángulo del servo 2
        ...
        // Llama a la función para ajustar los parámetros PID
        ajustar_pid();

        // Pausa para asegurar una frecuencia de 1 segundo
        delay_ms(1000);  // Retraso de 1 segundo para la siguiente lectura
    }
}
